<?php
    ...
    /x', $string, $matches, PREG_SET_ORDER);
        ...
        /**
         * Replaces constant identifiers.
         * Before Ilya's changes returns only strings. Now can replace array constants.
         *
         * @param  string $value the value to perform the replacement on
         *
         * @return mixed the value with substitutions made
         */
        public static function replaceConstants($value)
        {
            ...
            //try replace by array
            $match = $matches[0][1];
            $result = sfConfig::has(strtolower($match)) ? sfConfig::get(strtolower($match)) : "%{$match}%";
            if (is_string($result))
            {
                $result = str_replace($matches[0][0], $result, $value);
            }
        ...
        public static function getPhpCli()
        {
            ...
            $suffixes = DIRECTORY_SEPARATOR == '\\' ? (getenv('PATHEXT') ? explode(PATH_SEPARATOR, getenv('PATHEXT')) : array(
                '.exe',
                '.bat',
                '.cmd',
                '.com'

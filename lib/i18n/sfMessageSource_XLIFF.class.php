...
class sfMessageSource_XLIFF extends sfMessageSource_File
{
    ...
    public function update($text, $target, $comments, $catalogue = 'messages')
    {
        ...
        foreach ($units as $unit)
        {
            ...
            foreach ($unit->childNodes as $node)
            {
                ...
                {
                    $sourceValue = $node->firstChild instanceof DOMNode ? $node->firstChild->wholeText : '';
                    $found = ($sourceValue == $text);
                }
    ...
    public function updateByArray(array $texts, $catalogue = 'messages')
    {
        $variants = $this->getVariants($catalogue);
        ...
        foreach ($units as $unit)
        {
            ...
            foreach ($unit->childNodes as $node)
            {
                ...
                if ($node->nodeName == 'source')
                {
                    $key = $node->firstChild instanceof DOMNode ? $node->firstChild->wholeText : '';
                    $found = array_key_exists($key, $texts);
                }
